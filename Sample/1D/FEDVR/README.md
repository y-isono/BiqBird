...
| `H2_fedvr_HF.cpp` | 閉殻 H₂ 分子（核 2 個、$N_e = 2$ , $N_{\rm occ} = 1$ ）の RHF ポテンシャル曲線スキャン（§13） |
...
g++ -std=c++17 -O2 -I"$EIGEN" H2_fedvr_HF.cpp -o out/h2_fedvr_hf && ./out/h2_fedvr_hf > out/h2_fedvr_hf.csv
```
...
- **Phase C** : 多核（H₂ 分子）への拡張で `build_h_pq` を一般化（§13 で実装済み）
- **Phase B** : UHF 拡張で開殻 Li, H 原子に対応
- いずれも `hf_fedvr.hpp` の現在の RHF 実装と並列に追加できる

---

## 13. 多核外場：1D H₂ 分子のポテンシャル曲線（[`H2_fedvr_HF.cpp`](H2_fedvr_HF.cpp)）

Be で多軌道 RHF（$N_{\rm occ} > 1$）の動作確認をした次のステップとして、**多核外場**（核 2 個）に踏み込み、**閉殻 H₂ 分子** の RHF ポテンシャル曲線 $E_{\rm tot}(R)$ を計算する。`fedvr_basis.hpp` に追加した `Nucleus` 構造体と多核 `build_h_pq`、それに `nuclear_repulsion` を使い、`hf_fedvr.hpp` の SCF ソルバーをそのまま流用する。

### 13.1 物理モデル

- 核 2 個：$X_a = +R/2$, $X_b = -R/2$（核間距離 $R$）、それぞれ $Z_a = Z_b = 1$
- 電子数 $N_e = 2$, $N_{\rm occ} = 1$（閉殻）
- 1電子外場（ソフトクーロン、両核から）：

$$ V_{\rm ne}(x) = -\frac{1}{\sqrt{(x-R/2)^2 + 1}} - \frac{1}{\sqrt{(x+R/2)^2 + 1}} $$

- 核間反発（ソフトクーロン版、$a=1$）：

$$ E_{\rm nn}(R) = \frac{1}{\sqrt{R^2 + 1}} $$

- 全エネルギー：$E_{\rm tot}(R) = E_{\rm RHF}(R) + E_{\rm nn}(R)$

### 13.2 数値結果（$L = 20$, $N_e = 10$, $n = 8$）

| $R$ [a.u.] | $E_{\rm RHF}$ [Ha] | $E_{\rm nn}$ [Ha] | $E_{\rm tot}$ [Ha] |
|---|---|---|---|
| 0.4 | $-2.20254$ | $0.92848$ | $-1.27406$ |
| 0.6 | $-2.17672$ | $0.85749$ | $-1.31923$ |
| 0.8 | $-2.14254$ | $0.78087$ | $-1.36167$ |
| 1.0 | $-2.10159$ | $0.70711$ | $-1.39448$ |
| 1.2 | $-2.05557$ | $0.64018$ | $-1.41539$ |
| **1.4** | $-2.00619$ | $0.58124$ | **$-1.42496$** |
| **1.6** | $-1.95507$ | $0.53000$ | **$-1.42507$** ← 最低 |
| 1.8 | $-1.90362$ | $0.48564$ | $-1.41798$ |
| 2.0 | $-1.85294$ | $0.44721$ | $-1.40572$ |
| 2.5 | $-1.73338$ | $0.37139$ | $-1.36199$ |
| 3.0 | $-1.62724$ | $0.31623$ | $-1.31102$ |
| 4.0 | $-1.46634$ | $0.24254$ | $-1.22380$ |
| 5.0 | $-1.35664$ | $0.19612$ | $-1.16052$ |
| 6.0 | $-1.28493$ | $0.16440$ | $-1.12053$ |

各 $R$ で SCF はわずか 12〜13 反復で収束。

### 13.3 物理的解釈

- **平衡距離 $R_e \approx 1.6$ a.u.**：ソフトクーロン（$a = 1$）の 1D H₂ で、3D 実験値（$R_e = 1.401$ a.u.）よりやや大きい。これはソフトクーロンの引力が短距離で「丸まっている」ため、核近傍の電子局在が緩やかになり、結合距離が伸びる典型的な傾向。
- **結合エネルギー $D_e$**：解離極限の 2 個の独立 H 原子のエネルギーは $2 E_0^{\rm H} = 2 \times (-0.66977) = -1.33954$ Ha。一方、$R_e \approx 1.6$ で $E_{\rm tot} = -1.42507$ Ha なので、形式的には $D_e \approx 0.0855$ Ha $= 2.33$ eV。**ただし**、RHF の $R \to \infty$ 漸近値は $-1.12053$ Ha（$R = 6$）にしか到達せず、これは **RHF が左右の H 原子に電子 1 個ずつ局在させられない**ため。
- **RHF 解離問題が綺麗に見える**：$R \to \infty$ で「両核に半分ずつ広がった非物理的な電子分布」に閉じ込められて $E_{\rm tot}$ が真の解離極限 $-1.34$ より高止まりする。これは閉殻 RHF（単一空間軌道）の本質的な限界で、**UHF（α/β 別変分）か post-HF（CI/CC）が必要**になる物理的動機の最初の例。

### 13.4 次のステップ

- 同じデータが [`../FD/H2_fd_HF.cpp`](../FD/H2_fd_HF.cpp) でも得られる（FEDVR との比較は [`../README.md`](../README.md) 参照）
- 解離極限の RHF 破綻を「直す」には UHF（Phase B）または post-HF が必要
