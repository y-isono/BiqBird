...
$$
\hat H = \sum_{i=1}^{N_e}\left[-\frac{1}{2} \frac{d^2}{dx_i^2} - \sum_{a=1}^{N_n}\frac{Z_a}{\sqrt{(x_i - X_a)^2 + 1}}\right] + \sum_{i<j}\frac{1}{\sqrt{(x_i - x_j)^2 + 1}} + E_{\mathrm{nn}}
$$
...
$$
c_k = \tfrac{1}{2} (X_k + i Y_k) \prod_{j<k} Z_j, \quad c^\dagger_k = \tfrac{1}{2} (X_k - i Y_k) \prod_{j<k} Z_j
$$

「 $j<k$ の Z 文字列」（**JW string**）が反交換子関係 $\lbrace c_j, c_k^\dagger \rbrace = \delta_{jk}$ を保証する。
等価表現：

$$
c_k = \prod_{j<k} Z_j \cdot \sigma^-_k, \quad \sigma^-_k = \tfrac{1}{2} (X_k + i Y_k)
$$
...
$$
c^\dagger_{k_1} c_{k_2} + \mathrm{h.c.} = \tfrac{1}{2} (X_{k_1} X_{k_2} + Y_{k_1} Y_{k_2}) \prod_{k_{\min} < j < k_{\max}} Z_j
$$
...
2. 各 fermion 単項 $c^\dagger_a c^\dagger_b c_c c_d$（係数 $\alpha$ ）について、
   ...
   - JW 符号（ $(-1)^{\#\lbrace j: j < a, x_j = 1 \rbrace}$ 等）
