# 1-D Quantum (Phase Q): FEDVR DVR basis × Jordan-Wigner
...
| ③ 状態の表現 | $\|\psi\rangle \in \mathbb{C}^{2^n}$ | numpy 配列 / Qiskit Statevector | 古典シミュレータ（量子回路で扱う対象） |
...
iQPE プロトコルではラウンド $k$ で controlled-$U^{2^{n_{\rm bits}-1-k}}$ を ancilla 制御として作用させる必要がある。古典シミュレータでこれをそのまま実行すると `scipy.sparse.linalg.expm_multiply(-i τ · 2^{n-1-k} H, ψ)` を呼ぶことになり、$\tau \cdot 2^{n-1} \cdot \|H\|$ が大きいときに Krylov 部分空間の degree が増大して計算時間が現実的でなくなる。

`run_iqpe` では入力 $|\psi\rangle$ が $H$ の固有状態（直接対角化で得たもの）であることを利用し、固有関係

$$
U^{2^k}\,|\psi\rangle \;=\; e^{-i E\, 2^k \tau}\,|\psi\rangle
$$
...
$\hat H_{\rm el}$ は粒子数演算子 $\hat N$ と可換であり、$N_e$ 電子セクターは閉じた部分空間をなす。`trotter.evolve` はこの部分空間上で時間発展を計算するため、§4 で述べた粒子数射影 $H_{\rm sec} \in \mathbb{C}^{D \times D}$（$D = \binom{N_q}{N_e}$）を入力として受け取る。He 28 qubit でも $D = 378$ となり、古典シミュレータのメモリ制約内に収まる。
...
3. **iQPE 量子回路**: ancilla を $|+\rangle$ に置き、controlled-$U^{2^k}$（内部は Trotter 分解）、$R_z(-\omega_k)$、Hadamard、測定を $n_{\rm bits}$ 回繰り返す
...
$$
\hat H \;=\; \sum_{i=1}^{N_e}\left[-\tfrac12 \tfrac{d^2}{dx_i^2}
- \sum_{a=1}^{N_n}\frac{Z_a}{\sqrt{(x_i - X_a)^2 + 1}}\right]
+ \sum_{i<j}\frac{1}{\sqrt{(x_i - x_j)^2 + 1}}
+ E_{\rm nn}
$$

$E_{\rm nn} = \sum_{a<b} Z_a Z_b / \sqrt{(X_a - X_b)^2 + 1}$ は核間反発。
量子計算では $E_{\rm nn}$ は外部から足すスカラー、電子部分のみを qubit に載せる。

### 1.2 DVR 基底（FEDVR から流用）

[`../FEDVR/fedvr_basis.hpp`](../FEDVR/fedvr_basis.hpp) の `FEDVRGrid(L, n_elements, n_order)` が返す
**直交化済み DVR 基底** $\{\phi_p(x)\}_{p=1}^{N_{\rm DVR}}$ を空間軌道とする
（$N_{\rm DVR} = N_e^{\rm elem} \cdot n_{\rm order} - 1$）。
DVR の定義性質は

$$
\phi_p(x_q) \;=\; \delta_{pq}/\sqrt{w_p}, \qquad
\langle \phi_p | \phi_q \rangle \;\approx\; \delta_{pq}\quad (\text{GLL 求積})
$$

で、ノード座標 $\{x_p\}$ と DVR 重み $\{w_p\}$ がペアで揃う。
...
（運動エネルギー $T$ は FEDVR で疎・バンド幅 $\sim 2 n_{\rm order} + 1$；
核引力は対角）。これは `fedvr::build_h_pq(grid, nuclei)` で構築済み。

**2 電子積分** は DVR の局所性により 4 添字 $V_{pqrs}$ が 2 添字に **縮約**：

$$
V_{pqrs} \;=\; \int\!\!\int \phi_p(x_1)\phi_q(x_2)\,\frac{1}{\sqrt{(x_1-x_2)^2+1}}\,\phi_r(x_1)\phi_s(x_2)\,dx_1\,dx_2
\;\approx\; \delta_{pr}\delta_{qs}\,V_{pq}
$$

ここで

$$
V_{pq} \;=\; \frac{1}{\sqrt{(x_p - x_q)^2 + 1}}, \qquad V_{pp} = 1
$$

これが `fedvr::build_V_pq(grid)` の中身（$N_{\rm DVR}^2$ サイズの密行列で十分）。
**Gaussian 基底に比べて Pauli string 数が劇的に少ない** 主要因がこの縮約である。

### 1.4 第二量子化 Hamiltonian

スピン軌道 $|\phi_p \otimes \sigma\rangle$（$\sigma \in \{\alpha, \beta\}$）に
...
\boxed{\,
\hat H_{\rm el} \;=\;
\sum_{p,q,\sigma} h_{pq}\, c^\dagger_{p\sigma} c_{q\sigma}
\;+\; \tfrac12 \sum_{p,q}\sum_{\sigma,\sigma'} V_{pq}\,
c^\dagger_{p\sigma}\, c^\dagger_{q\sigma'}\, c_{q\sigma'}\, c_{p\sigma}
\,}
$$

$p = q$ かつ $\sigma = \sigma'$ の項は Pauli 排他律により $c^\dagger c^\dagger = 0$ で自動消滅。
全エネルギーは $E_{\rm tot} = E_{\rm el} + E_{\rm nn}$。
...
OpenFermion の分子規約に揃え、空間軌道 $p \in \{0,\dots,N_{\rm DVR}-1\}$ と
スピン $s \in \{0(\alpha), 1(\beta)\}$ を、フラットなフェルミオンモード番号 $k$ に：

$$
\boxed{\, k \;=\; 2p + s \,}\qquad(0 \le k < 2 N_{\rm DVR})
$$
...
$$
|0\rangle_k \;\equiv\; |\text{vacuum on mode } k\rangle, \qquad
|1\rangle_k \;\equiv\; c^\dagger_k |0\rangle_k
$$
...
$$
N_q \;=\; 2\, N_{\rm DVR} \;=\; 2\,(N_e^{\rm elem}\, n_{\rm order} - 1)
$$
...
$$
\hat N \;=\; \sum_k c^\dagger_k c_k \;=\; \sum_k \frac{1 - Z_k}{2}
\;=\; \frac{N_q}{2} - \tfrac12 \sum_k Z_k
$$

計算基底状態に作用すると **bit-popcount**：$\hat N |x\rangle = \mathrm{popcount}(x)\,|x\rangle$。
電子数 $N_e$ セクターは $\binom{N_q}{N_e}$ 次元で、$\hat H_{\rm el}$ は粒子数を保存する
$([\hat H_{\rm el}, \hat N] = 0)$ ので、各セクターでブロック対角。
これがメモリ最適化（§4）の基礎。
...
$$
\boxed{\;
c_k \;=\; \tfrac12 (X_k + i Y_k) \prod_{j<k} Z_j, \quad
c^\dagger_k \;=\; \tfrac12 (X_k - i Y_k) \prod_{j<k} Z_j
\;}
$$

「$j<k$ の Z 文字列」（**JW string**）が反交換子関係 $\{c_j, c_k^\dagger\} = \delta_{jk}$ を保証する。
等価表現：

$$
c_k \;=\; \prod_{j<k} Z_j \cdot \sigma^-_k, \qquad
\sigma^-_k = \tfrac12 (X_k + i Y_k)
$$

### 3.2 1-body 項のJW像

$h_{pq} c^\dagger_{p\sigma} c_{q\sigma}$ を JW で書き下す。
モード番号 $k_1 = 2p+s$、$k_2 = 2q+s$、$k_{\rm min} = \min(k_1, k_2)$、$k_{\rm max} = \max(k_1, k_2)$ として、

- **$p = q$（対角）**：$c^\dagger_k c_k = \frac{1 - Z_k}{2}$ → 1 個の $Z$ + 定数
- **$p \neq q$（オフ対角、Hermite 化済み）**：

$$
c^\dagger_{k_1} c_{k_2} + \mathrm{h.c.} \;=\;
\tfrac12 (X_{k_1} X_{k_2} + Y_{k_1} Y_{k_2}) \prod_{k_{\rm min} < j < k_{\rm max}} Z_j
$$
...
$$
c^\dagger_p c^\dagger_q c_q c_p = \hat n_p \hat n_q
\;=\; \frac{1-Z_p}{2}\cdot\frac{1-Z_q}{2}
\;=\; \tfrac14 (I - Z_p - Z_q + Z_p Z_q)
$$
...
$$
\tfrac12 V_{pq}\sum_{\sigma\sigma'} \hat n_{p\sigma} \hat n_{q\sigma'}
\;=\; \tfrac12 V_{pq}\,(\hat n_{p\alpha} + \hat n_{p\beta})(\hat n_{q\alpha} + \hat n_{q\beta})
$$
...
| 系 | $N_{\rm DVR}$ | $N_q$ | 1-body Pauli string | 2-body Pauli string | 合計 |
|---|---|---|---|---|---|
...
$\hat H_{\rm el}$ は粒子数 $\hat N$ と可換なので、$N_e$ セクター上で完全に閉じる。
セクター次元は二項係数

$$
D \;=\; \binom{N_q}{N_e} \;=\; \binom{2 N_{\rm DVR}}{N_e}
$$
...
DVR セクター行列 $H_{\rm sec} \in \mathbb{C}^{D \times D}$（実 Hermitian）の
最低固有値・固有ベクトルを求める：

$$
H_{\rm sec}\, v_0 \;=\; E_0\, v_0, \qquad
E_0 \;=\; \min \mathrm{spec}(H_{\rm sec})
$$
...
| $> 4096$ | sparse `scipy.sparse.linalg.eigsh(k=1, which="SA")`（Lanczos） |
...
$n_{\rm bits}$ 桁の 2 進小数で測定する。標準 QPE は QFT register
（$n_{\rm anc}$ qubit ancilla）を使い

$$
|0\rangle^{\otimes n_{\rm anc}} |\psi\rangle
\xrightarrow{H^{\otimes n_{\rm anc}}}
\frac{1}{\sqrt{2^{n_{\rm anc}}}}\sum_{m=0}^{2^{n_{\rm anc}}-1} |m\rangle |\psi\rangle
\xrightarrow{\text{controlled-}U^m}
\frac{1}{\sqrt{2^{n_{\rm anc}}}}\sum_m e^{-2\pi i m\phi} |m\rangle |\psi\rangle
\xrightarrow{\mathrm{QFT}^\dagger}
|\tilde m\rangle |\psi\rangle, \quad \tilde m \approx 2^{n_{\rm anc}}\phi
$$

メモリ：状態ベクトルは $2^{N_{\rm sys} + n_{\rm anc}}$ 次元 → He 28 qubit + 8 bit ancilla で 1 TB
→ **24 GB PC ではアウト**（[`quantum_computing_plan.md`](../../../BiqBird_Doc/quantum_computing_plan.md) §2.3 参照）。

### 6.2 Iterative QPE（iQPE）

ancilla を **1 qubit に削減**し、$n_{\rm bits}$ ラウンドの逐次測定 + 古典フィードバックで
同じ精度を得る（Kitaev / Dobšíček 2007）。
本実装は **LSB-first**：bit $b_0$（LSB）を最初に、$b_{n_{\rm bits}-1}$（MSB）を最後に測る。

#### 6.2.1 1 ラウンドの量子回路

ラウンド $k$（$k = 0, \dots, n_{\rm bits} - 1$）で `power_k = 2^{n_{\rm bits}-1-k}` を使う：
...
\xrightarrow{H_{\rm anc}}
\frac{1}{\sqrt2}(|0\rangle + |1\rangle)\otimes|\psi\rangle
$$

$$
\xrightarrow{C\text{-}U^{\rm power_k}}
\frac{1}{\sqrt2}\bigl(|0\rangle\otimes|\psi\rangle + |1\rangle\otimes U^{\rm power_k}|\psi\rangle\bigr)
$$

固有状態前提（$U^k|\psi\rangle = e^{-iEk\tau}|\psi\rangle$、$\theta_k = -E\tau\cdot\text{power}_k$）：

$$
= \frac{1}{\sqrt2}(|0\rangle + e^{i\theta_k}|1\rangle)\otimes|\psi\rangle
$$

フィードフォワード位相補正 $R_z(-\omega_k)$ on ancilla（$|1\rangle$ 側に $e^{-i\omega_k}$）：

$$
\to \frac{1}{\sqrt2}(|0\rangle + e^{i(\theta_k - \omega_k)}|1\rangle)\otimes|\psi\rangle
$$
...
$$
\xrightarrow{H_{\rm anc}}
\Bigl[\cos\tfrac{\theta_k - \omega_k}{2}|0\rangle
+ i\sin\tfrac{\theta_k - \omega_k}{2}|1\rangle\Bigr]\otimes|\psi\rangle
$$

測定確率：

$$
\boxed{\;P(\text{ancilla} = 0) \;=\; \cos^2\!\Bigl(\tfrac{\theta_k - \omega_k}{2}\Bigr)\;}
$$

#### 6.2.2 フィードフォワード補正の導出

$\phi$ が **正確に $n_{\rm bits}$ 桁の二進小数** $\phi = m/2^{n_{\rm bits}}$
（$m = \sum_j b_j 2^j$）と仮定する。
ラウンド $k$ で：

$$
\theta_k = -2\pi\phi \cdot 2^{n_{\rm bits}-1-k} \mod 2\pi
\;=\; -\pi b_k - 2\pi\!\!\sum_{j=0}^{k-1}\!\! b_j\,2^{j-k-1} \pmod{2\pi}
$$

導出：$\phi \cdot 2^{n_{\rm bits}-1-k} = m/2^{k+1} = b_k/2 + \sum_{j<k} b_j 2^{j-k-1} + (\text{整数})$
...
$$
\boxed{\;\omega_k \;=\; -\pi \sum_{j=0}^{k-1} b_j\, 2^{j-k} \pmod{2\pi}\;}
$$

これで $\theta_k - \omega_k = -\pi b_k$、よって

$$
P(0) = \cos^2(-\tfrac{\pi b_k}{2}) =
\begin{cases} 1 & b_k = 0 \\ 0 & b_k = 1 \end{cases}
$$
...
総計 **$O(n_{\rm bits})$ 古典計算 + 1 sparse mat-vec**（最初の $E$ 計算のみ）。
量子コンピュータ実機なら $O(2^{n_{\rm bits}})$ ゲート（controlled-$U^{2^{n-1}}$ が一番重い）。

#### 6.2.4 spectrum shift（`shifted_for_iqpe`）

iQPE は $\phi \in [0,1)$ を仮定する。
He の最低固有値 $E_0 \approx -2.1$ Ha は負だし、
$|E\tau| < 2\pi$ も保証する必要がある。
そこで cheap eigsh で $[E_{\rm min}, E_{\rm max}]$ を粗推定して

$$
\hat H' = \hat H + \mathrm{shift}\cdot I, \quad
\mathrm{shift} = -E_{\rm min} + \mathrm{margin}\cdot(E_{\rm max} - E_{\rm min})
$$

$$
\tau = \frac{2\pi}{(E_{\rm max} + \mathrm{shift})(1 + \mathrm{margin})}
$$

として、シフト済み固有値 $E'_0 = E_0 + \mathrm{shift}$ から
$E_0 = E_{\rm iqpe} - \mathrm{shift}$ で復元。
margin = 0.05 を採用し、`shifted_for_iqpe(H, margin=0.05)` を呼ぶ。
...
| `run_iqpe_general` | 任意状態 | $n_{\rm bits}$ 個の `expm_multiply` | Q4 以降の VQE 連携 |
...
固有状態前提なら $U^{\rm power} |\psi\rangle = e^{-iE\tau\cdot\rm power}|\psi\rangle$
...
$$
\Delta E \;=\; \frac{2\pi}{\tau \cdot 2^{n_{\rm bits}}}
$$

$n_{\rm bits} = 14$、He 18 qubit（$\tau \sim 1.5$）で $\Delta E \sim 2.5 \times 10^{-4}$ Ha。
3 桁追加で $n_{\rm bits} = 24$ にすれば $\sim 2.4 \times 10^{-7}$ Ha まで取れるが、
**古典シミュレータ的には何 bit でも瞬時**（fast path のため）。
量子実機では $n_{\rm bits}$ ごとにコヒーレンス時間が $2 \times$ 必要なので
...
**$\hat T$ は 1-body の $p \neq q$ 項のみ**、しかもバンド幅 $\sim 2 n_{\rm order} + 1$。

### 7.2 2nd-order Strang splitting

$$
\boxed{\;
U(\Delta t) \;\approx\; e^{-i \hat T \Delta t/2}\, e^{-i \hat V \Delta t}\, e^{-i \hat T \Delta t/2}
\;}
$$
...
| 因子 | 演算 | 量子ゲート数 |
|---|---|---|
| $e^{-i V_{\rm ne}(x_p) \hat n_p \Delta t}$（1-body 対角）| $R_z$ | $N_q$ |
...
$T_{\rm total} = 5$ a.u. の伝搬を $\Delta t = 0.02$（250 step）でも 0.1 秒。
...
$$
U_4(\Delta t) = U_2(s\Delta t)^2 U_2((1-4s)\Delta t) U_2(s\Delta t)^2, \quad
s = \frac{1}{4 - 4^{1/3}}
$$
...
| Step | $L$ | $N_e^{\rm elem}$ | $n_{\rm order}$ | $N_{\rm DVR}$ | $N_q$ | sector dim | $E_{\rm classical}$ [Ha] | $E_{\rm direct}$ [Ha] | $E_{\rm iqpe}$ [Ha] | iQPE \|Δ\| [Ha] |
|---|---|---|---|---|---|---|---|---|---|---|
...
| Step | $L$ | $N_e^{\rm elem}$ | $n_{\rm order}$ | $N_{\rm DVR}$ | $N_q$ | sector dim | $E_{\rm FCI}$ [Ha] | $E_{\rm iqpe}$ [Ha] | iQPE \|Δ\| | RSS |
|---|---|---|---|---|---|---|---|---|---|---|
...
**観察**：
- iQPE（14 bit、resolution $\sim 5\times 10^{-4}$ Ha）は全 Step で $E_{\rm FCI}$ と一致
...
設定：$L=10, N_e^{\rm elem}=2, n_{\rm order}=5 \Rightarrow N_{\rm DVR}=9, N_q=18$, sector dim = 153

| $R$ [a.u.] | $E_{\rm FCI}$ | $E_{\rm iqpe}$ | $E_{\rm nn}$ | $E_{\rm tot}^{\rm Q}$ | $E_{\rm tot}^{\rm RHF}$ | $\Delta E_{\rm corr}$ |
|---|---|---|---|---|---|---|
...
$\Delta E_{\rm corr} = E_{\rm tot}^{\rm RHF} - E_{\rm tot}^{\rm Q}$（正なら量子が下回る）。

**観察**：
- $R = 1.0$ で量子が上に位置するのは古典 RHF（$N_{\rm DVR}=79$ の細かいグリッド）vs
  量子（$N_{\rm DVR}=9$ の粗いグリッド）のグリッド差
- $R \ge 1.6$ で量子が一貫して下回り、**$R = 6.0$ では $0.31$ Ha の差**：
  これが教科書的な「RHF 解離破綻 vs FCI/quantum の正しい解離」のデモ
- 解離極限 $2 E_0^{\rm H} \approx -1.34$ Ha に対し量子が $-1.43$（やや深め=$N_{\rm DVR}=9$ の粗さで H 原子が深い）、
  RHF は $-1.12$ で大きく上に詰まる

### 9.4 Phase Q4 : Trotter 時間発展（GS のエネルギー保存）

設定：He Step 6（18 qubit）の基底状態を $T_{\rm total} = 5$ a.u. 伝搬。
2nd-order Strang splitting:

$$
U(\Delta t) = e^{-i T \Delta t/2}\, e^{-i V \Delta t}\, e^{-i T \Delta t/2}
$$

| $\Delta t$ | n_steps | $\max |\Delta E|$ [Ha] | final overlap | scaling 比 |
|---|---|---|---|---|
...
3. **controlled-$U^{\rm power}$**: シフト済み Hamiltonian の対角化 $H = V\,\mathrm{diag}(E_n)\,V^\dagger$ から $V\,\mathrm{diag}(e^{-i E_n \tau \cdot \mathrm{power}})\,V^\dagger$ を構成し、`UnitaryGate` として `.control(1)` で ancilla 制御化する。実機向けに展開する場合はこのユニタリを §7 の Trotter 分解で Pauli string ごとのゲート列に置き換える。
...
`uv run python scripts/q5_qiskit_reference.py` の実行結果（H Step 1, $n_{\rm bits} = 12$）：
...
| 1 ラウンドの controlled-$U^{\rm power}$ に必要な Trotter step 数 | $\tau \cdot \mathrm{power} / \Delta t$（$\tau = 1.5$, power $= 2^{13}$, $\Delta t = 0.05$ で $\sim 5 \times 10^5$）|
| 1 ラウンドの累積ゲート数 | $\sim 1.6 \times 10^8$（最大 power のラウンド）|
| 全 14 ラウンドの累積ゲート数 | $\sim 3 \times 10^8$ |
| 必要なコヒーレンス時間 | $T_{\rm gate} \times 3 \times 10^8 \sim 30$ 秒（$T_{\rm gate} = 100$ ns 仮定）|
