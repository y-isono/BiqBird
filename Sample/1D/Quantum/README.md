...
iQPE プロトコルではラウンド $k$ で controlled-$U^{2^{n_{\mathrm bits}-1-k}}$ を ancilla 制御として作用させる必要がある。古典シミュレータでこれをそのまま実行すると `scipy.sparse.linalg.expm_multiply(-i τ · 2^{n-1-k} H, ψ)` を呼ぶことになり、$\tau \cdot 2^{n-1} \cdot \|H\|$ が大きいときに Krylov 部分空間の degree が増大して計算時間が現実的でなくなる。
...
$E_{\mathrm nn} = \sum_{a<b} Z_a Z_b / \sqrt{(X_a - X_b)^2 + 1}$ は核間反発。
...
**直交化済み DVR 基底** $\{\phi_p(x)\}_{p=1}^{N_{\mathrm DVR}}$ を空間軌道とする
（$N_{\mathrm DVR} = N_e^{\mathrm elem} \cdot n_{\mathrm order} - 1$）。
DVR の定義性質は

$$
\phi_p(x_q)  =  \delta_{pq}/\sqrt{w_p}, quad
...
OpenFermion の分子規約に揃え、空間軌道 $p \in \{0,\dots,N_{\mathrm DVR}-1\}$ と
スピン $s \in \{0(\alpha), 1(\beta)\}$ を、フラットなフェルミオンモード番号 $k$ に：

$$
  k  =  2p + s  }quad(0 \le k < 2 N_{\mathrm DVR})
$$
...
$$
|0\rangle_k  \equiv  |\text{vacuum on mode } k\rangle, quad
...
$$
N_q  =  2  N_{\mathrm DVR}  =  2 (N_e^{\mathrm elem}  n_{\mathrm order} - 1)
$$
...
$$
c_k  =  \prod_{j<k} Z_j \cdot \sigma^-_k, quad
...
モード番号 $k_1 = 2p+s$、$k_2 = 2q+s$、$k_{\mathrm min} = \min(k_1, k_2)$、$k_{\mathrm max} = \max(k_1, k_2)$ として、
...
\frac12 (X_{k_1} X_{k_2} + Y_{k_1} Y_{k_2}) \prod_{k_{\mathrm min} < j < k_{\mathrm max}} Z_j
$$
...
$$
H_{\mathrm sec}  v_0  =  E_0  v_0, quad
E_0  =  \min \mathrm{spec}(H_{\mathrm sec})
$$
...
ラウンド $k$（$k = 0, \dots, n_{\mathrm bits} - 1$）で `power_k = 2^{n_{\mathrm bits}-1-k}` を使う：
...
$$
\xrightarrow{C\text{-}U^{\mathrm power_k}}
\frac{1}{\sqrt2}\bigl(|0\rangle\otimes|\psi\rangle + |1\rangle\otimes U^{\mathrm power_k}|\psi\rangle\bigr)
$$
...
$$
 P(\text{ancilla} = 0)  =  \cos^2(\frac{\theta_k - \omega_k}{2})
$$
...
$$
\theta_k = -2\pi\phi \cdot 2^{n_{\mathrm bits}-1-k} \mod 2\pi
 =  -\pi b_k - 2\pi\!\!\sum_{j=0}^{k-1}\!\! b_j 2^{j-k-1} \pmod{2\pi}
$$

導出：$\phi \cdot 2^{n_{\mathrm bits}-1-k} = m/2^{k+1} = b_k/2 + \sum_{j<k} b_j 2^{j-k-1} + (\text{整数})$
...
$$
 \omega_k  =  -\pi \sum_{j=0}^{k-1} b_j  2^{j-k} \pmod{2\pi}
$$
...
\mathrm{shift} = -E_{\mathrm min} + \mathrm{margin}\cdot(E_{\mathrm max} - E_{\mathrm min})
$$
...
$E_0 = E_{\mathrm iqpe} - \mathrm{shift}$ で復元。
...
固有状態前提なら $U^{\mathrm power} |\psi\rangle = e^{-iE\tau\cdot\mathrm power}|\psi\rangle$
